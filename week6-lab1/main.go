...
func createStudent(c *gin.Context) {
	...
	if newStudent.Name == "" {
		c.JSON(http.StatusBadRequest, gin.H{"name": "name is required"})
		return
	}

	if newStudent.Year < 1 || newStudent.Year > 4 {
		c.JSON(http.StatusBadRequest, gin.H{"year": "year must be between 1 and 4"})
		return
	}

	if newStudent.GPA < 0 || newStudent.GPA > 4 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "gpa must be between 0 and 4"})
		return
	}
